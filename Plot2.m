...
current = fopen('E:\Piotr\Programowanie\C#\AISDE2\Lab2Console\Lab2Console\PlotDataTime.txt'); % otwiera plik
[ctime com] = fscanf(current, '%g ' ); % wczytuje do macierzy A
if(ctime<0)
  disp(com);  
end
...
plot(ctime(1:2:length(ctime)),A);
figure (2);
plot(ctime(1:(length(ctime)-1)),v(2:length(v)));